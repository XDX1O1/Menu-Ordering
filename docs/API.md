...
| AUTH_002 | Session expired | User session has expired |
| AUTH_003 | Unauthorized access | User doesn't have required role |
| ORDER_001 | Invalid order data | Order data validation failed |
| ORDER_002 | Order not found | Order ID/number doesn't exist |
| ORDER_003 | Cannot modify order | Order is in final state |
| PAYMENT_001 | Insufficient payment | Payment amount less than total |
| PAYMENT_002 | Payment already processed | Order already paid |
| MENU_001 | Menu not found | Menu ID doesn't exist |
| MENU_002 | Menu not available | Menu item is not available |
| CART_001 | Empty cart | Cart has no items |
| CART_002 | Invalid quantity | Quantity must be positive |
...
1. Import collection from `docs/postman/ChopChop-API.postman_collection.json` (if available)
2. Set environment variable: `baseUrl = http://localhost:8080`
3. Login to get session cookie
4. Use saved session for subsequent requests

## API Versioning

Currently: **Version 1** (no prefix)

Future versions may use URL prefix:
- `/api/v1/...`
- `/api/v2/...`
